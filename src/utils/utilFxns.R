...
ClusterSeurat = function(object, nfeatures = 2000, numPCs = 20, normalization.method = "LogNormalize", scale.factor = 10000, selection.method = "vst",  
                         ...
                         remove.batches = 60, reduction = "cca", k.anchor = 5, normalize = TRUE){
  if(!is.null(integrate.by)){
    ...
    for (i in 1:length(obj.list)) {
      if(normalize) obj.list[[i]] <- NormalizeData(obj.list[[i]], verbose = FALSE)
    ...
    if(normalize) object <- NormalizeData(object)
