PredictLabels2 = function(test, model, test_ident = NULL, scale = TRUE, scale.by.model = FALSE, assay = "RNA", slot = "data", verbose = FALSE, 
  ...
  # Make sure gene order is the same
  test_xgb = t(test_data[genes.use,])
  
  # Check dims
  if(verbose) message('Test data has dimensions: ', paste0(dim(test_xgb), collapse = ' x '))
  ...
  test_prediction <- matrix(test_pred, 
                            ...
                            byrow = FALSE) # should this be on? I don't think so 
  
  rownames(test_prediction) = levels(train_id)
  colnames(test_prediction) = colnames(test_data)
  if(verbose) print(test_prediction)
  ...
  test$xgboost.labels = test_pred_names
  test$xgboost.prob = test_pred_margins
  
  if(return.prob.matrix) return(test_prediction)
...
XGBoost_train2 = function(X_train, X_test, predict.by = 'seurat_clusters', 
                          assay = 'RNA', nround = 400, scale = TRUE, 
                          scale.mean = NULL, scale.var = NULL, 
                          alpha.value = 0, 
  ...
  # Features are inherited by scaled data
  features.use = rownames(X_train@assays[[assay]]@scale.data)
  if(scale) {
    dat.train = X_train@assays[[assay]]@scale.data[features.use,]
    dat.test = X_test@assays[[assay]]@scale.data[features.use,]
  } else {
    dat.train = X_train@assays[[assay]]@data[features.use,]
    dat.test = X_test@assays[[assay]]@data[features.use,]
  }
  stopifnot(nrow(dat.train) > 0)
  message('Using ', nrow(dat.train), ' genes!')
  train_matrix <- xgb.DMatrix(data = t(dat.train), label=train_labels)
  validation_matrix <- xgb.DMatrix(data = t(dat.test), label=test_labels)
...
#' do.scale = whether we want to z-score the features
#' train.frac = fraction of cells in each ident we want to use for training
XGBoost_train = function(train_Data, train_labels = NULL, var.genes=NULL, 
                         do.scale=FALSE, scale.mean = NULL, scale.var = NULL, 
                         max.cells.per.ident = 400, train.frac = 0.6, nround = 200){
  library(xgboost)
...
#' @examples
#' adult_to_larva <- BuildConfusionMatrix(larva, adult, model = larva.model)
PredictLabels = function(test, model, test_ident = NULL, scale = TRUE, 
                         scale.by.model = FALSE, assay = "RNA", slot = "data", verbose = TRUE){
  genes.use <- model$bst_model$feature_names
  train_id = factor(colnames(model$test_mat), levels = unique(colnames(model$test_mat)))
  ...
  # Ensure that it's in the right order for genes
  test_xgb = t(test_data[genes.use,])
